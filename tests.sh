...
#checkinstall -D -y --exclude=build --pkggroup=main --maintainer=stefan@eonerc.rwth-aachen.de --pkgsource=https://hermitcore.org --pkgname=libhermit --pkgversion=0.2.11 --pkglicense=BSD make hermit-bootstrap-install
...
cd ..
mkdir -p tmp
dpkg-deb -R build/libhermit-0.2.11-all.deb tmp
#dpkg-deb -R build/libhermit_0.2.11-1_amd64.deb tmp
