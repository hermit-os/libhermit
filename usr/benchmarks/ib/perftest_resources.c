...
int run_iter_lat_write(struct pingpong_context *ctx,struct perftest_parameters *user_param)
{
	...
	uint64_t ccnt = 0; // Complete sent packets counter
	...
	poll_buf = (char*)ctx->buf[0] +
	           (user_param->num_of_qps + poll_buf_offset)*BUFF_SIZE(ctx->size, ctx->cycle_buffer) +
	           user_param->size - 1;

	// While any type of packet is still missing
	while (scnt < user_param->iters || ccnt < user_param->iters || rcnt < user_param->iters) {
		// If there are still packets to receive + at least 1 packet has been sent or this is the client.
		...
		// Post work request if there are packets left to be sent.
		...
		// If there are packets left to be completed, poll completion queue
