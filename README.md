...
The project [HermitCore](http://www.hermitcore.org) is a new [unikernel](http://unikernel.org) targeting at high-performance computing.
HermitCore extends the multi-kernel approach with unikernel features for a better programmability and scalability for hierarchical systems.
...
3. The command `make` builda the Linux kernel, the HermitCore kernel, the corss-compiler, and the demo applications.
...
3. Install the Linux kernel and its initial ramdisk on your system (see descriptions of your Linux distribution). We recommend to disable Linux NO_HZ feature by setting the kernel parameter `no_hz=off`. This feature partly reduces the OS noise, but noise sensitive applications should now run on HermitCore.
